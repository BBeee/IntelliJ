...
//  implicit val tsrWrites = Json.writes[TimeSeriesResponse]
...
//  implicit val tsdrWrites = Json.writes[TimeSeriesDailyResponse]
...
class AlphaVantageApi(apikey: String)(implicit system: ActorSystem, materializer: Materializer, ex: ExecutionContext) extends PlayJsonSupport {
  ...
  def minute(symbol: String, outputsize: String = "compact")(implicit um: Reads[AlphaVantage.TimeSeriesResponse]) =
    http(baseUrl + s"TIME_SERIES_INTRADAY&interval=1min&symbol=${symbol}&outputsize=${outputsize}&apikey=${apikey}").flatMap(x => unmarshal(x) )
